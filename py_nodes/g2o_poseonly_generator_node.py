...
class G2oPoseOnlyGeneratorNode(Node):
    def __init__(self):
        super().__init__('g2o_poseonly_generator_node')
        ...
        self.declare_parameter('k_dist_factor', 0.02)    # scaling factor for measurement variance based on tag distance (k * distance^2)
        self.declare_parameter('alpha1', 0.15)  # motion noise related to forward velocity (err per meter)
        self.declare_parameter('alpha2', 0.15)  # motion noise related to angular velocity (err per radian)
        self.declare_parameter('alpha3', 0.10)  # motion noise related to forward velocity
        self.declare_parameter('alpha4', 0.10)  # motion noise related to angular velocity 
        ...
        self.prev_mu = None
        self.tag_observations = {}   # list of (pose_id, x_rel, y_rel, theta_rel) from all poses that have observerd this tag (keyed by tag_id)
        self.pose_to_tags = {}       # pose_id -> set of tag_ids already processed from this pose
        self.loop_closure_threshold = 6 # threshold to only add loop closure between far poses (current.id - old.id)
        self.g2o_file = open('generated_poseonly_graph.g2o', 'w')  # Output g2o file
        ...
        self.gtruth_file = open('groundtruth_poseonly.txt', 'w')
        ...
        self.k = float(self.get_parameter('k_dist_factor').value)
    ...
    def generate_from_odom(self):
        ...
        # Information matrix based on alpha parameters and motion magnitude (greater motion = more uncertainty)
        trans = sqrt(dx_rel**2 + dy_rel**2)
        rot = abs(dtheta_rel)
        var_xy = (self.alpha1 * trans + self.alpha2 * rot) ** 2
        var_theta = (self.alpha3 * trans + self.alpha4 * rot) ** 2
        cov = np.diag([var_xy, var_xy, var_theta])
        info_matrix = np.linalg.inv(cov + 1e-12 * np.eye(3))
    ...
    def process_tag_measurements(self):
        ...
        current_pose_id = self.pose_id
        if current_pose_id not in self.pose_to_tags:
            self.pose_to_tags[current_pose_id] = set()
        
        for tag_id in self.detected_tag_ids:
            if tag_id in self.pose_to_tags[current_pose_id]:  # skip if already processed this tag from this pose to avoid duplicate edges
                self.get_logger().debug(f'Detected tag {tag_id} already processed for this pose, skipping.', throttle_duration_sec=2.0)
                continue

            self.pose_to_tags[current_pose_id].add(tag_id)  # mark this tag as processed for current pose

            tag_frame = tag_id
            try:
                t = self.tf_buffer.lookup_transform(self.base_frame, tag_frame, rclpy.time.Time())
                cur_x, cur_y, cur_theta = self.tf_to_se2(t)
                
                # loop closure
                if tag_id not in self.tag_observations:
                    self.tag_observations[tag_id] = []
                    self.get_logger().debug(f'First observation of {tag_id} at pose {current_pose_id}')
                
                for old_pose_id, old_x, old_y, old_theta in self.tag_observations.get(tag_id, []):
                    # check for loop closure (sufficient pose id gap)
                    if current_pose_id - old_pose_id > self.loop_closure_threshold:
                        dx, dy, dtheta = self.relative_from_common_tag(old_x, old_y, old_theta, cur_x, cur_y, cur_theta)
                                            
                        # Info matrix for tag measurement edge based on distance and scaling by k
                        tag_dist = sqrt(cur_x**2 + cur_y**2)
                        var_xy = self.default_var_xy + self.k * tag_dist**2
                        var_theta = self.default_var_theta + self.k * tag_dist**2
                        info_matrix = np.diag([1.0 / var_xy, 1.0 / var_xy, 1.0 / var_theta])
                        
                        self.write_edge_se2(old_pose_id, current_pose_id, np.array([dx, dy, dtheta]), info_matrix)
                        self.get_logger().info(f'Loop closure via {tag_id} between pose {old_pose_id} and {current_pose_id}')
                
                # Append current observation to the list for this tag (so future poses can loop close to it)
                self.tag_observations[tag_id].append((current_pose_id, cur_x, cur_y, cur_theta))
    ...
    def tf_to_se2(self, t):
        """Convert TransformStamped (base -> tag) to x, y, theta in robot frame."""
        tr = t.transform.translation
        rot = t.transform.rotation
        theta = quat_to_yaw(rot)
        return tr.x, tr.y, theta

    def se2_to_homog(self, x, y, theta):
        """2D pose -> 3x3 homogeneous matrix."""
    ...
    def relative_from_common_tag(self, old_x, old_y, old_theta, curr_x, curr_y, curr_theta):
        """Compute relative pose (old_pose -> curr_pose) using the same tag as intermediate."""
        T_old_tag = self.se2_to_homog(old_x, old_y, old_theta)
        T_curr_tag = self.se2_to_homog(curr_x, curr_y, curr_theta)
        T_old_curr = T_old_tag @ np.linalg.inv(T_curr_tag)
        dx = T_old_curr[0, 2]
        dy = T_old_curr[1, 2]
        dtheta = atan2(T_old_curr[1, 0], T_old_curr[0, 0])
        return dx, dy, wrap_angle(dtheta)


def main(args=None):
    rclpy.init(args=args)
    node = G2oPoseOnlyGeneratorNode()
    try:
        rclpy.spin(node)
